...
- `listen_port` is the port the server binds inside the guest. The socket it gets is the one the manager passed; the guest cannot open additional listening ports on the host.
