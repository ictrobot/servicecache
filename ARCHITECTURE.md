...
- **Freeze is terminal.** The host waits until every guest thread is suspended in a wait, ends every thread but the control thread, and verifies the process has one thread. A frozen host is a template: it answers only `fork` and `stop`.
